...
class Volume(SourceBase):
    ...
    def create(plugin, volume_size, display_name, display_description, volume_type=None):
        response_data = plugin.cinder.create_volume(volume_size, display_name, display_description, volume_type=volume_type)
    ...
    def create_instance(self, name, flavor_id, volume_size, net_ids):
        ...
        response_data = self.plugin.nova.volumes_boot(name, flavor_id, 'volume', self.source_id, volume_size, net_ids)
    ...
    def create_snapshot(self, until_done=False):
        ...
        response_data = self.plugin.cinder.create_snapshot_from_volume(self.source_id)
    ...
    def create_another_env_image(self, another_plugin, disk_format=None):
        ...
        response_data = self.plugin.cinder.create_another_env_image(
            ...
            container_format=self.volume_obj.image_container_format, # CONF.DEFAULT_CONTAINER_FORMAT
            disk_format=disk_format or self.volume_obj.image_disk_format, # CONF.DEFAULT_DISK_FORMAT
        )
